...
## How Non-Selected Tokens And Probabilities Are Retrieved (OpenAI)

For each generation step, the app requests exactly one next token from OpenAI and asks OpenAI to also return alternative token candidates at that same position.

- The request uses:
  - `max_completion_tokens: 1` (single-step generation)
  - `logprobs: true`
  - `top_logprobs: 20`
- OpenAI returns:
  - the sampled/returned token
  - the top alternative tokens with log probabilities (`top_logprobs`)
- The app converts each token `logprob` to probability with `exp(logprob)`.
- The selected branch is chosen according to the active strategy (`Top-k`, `Top-p`, or `Temperature`), while the rest are displayed as non-selected branches.

Official OpenAI docs for this behavior:
- [Chat Completions API (`logprobs`, `top_logprobs`)](https://platform.openai.com/docs/api-reference/chat/create#chat-create-logprobs)
...
- `src/components/visualizer-app.tsx`
  - Top-level composition: control panel, canvas, selected path preview
